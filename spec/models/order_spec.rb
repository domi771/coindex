...
describe Order, 'validations' do
  ...
  context "limit order" do
    it "should make sure price is present" do
      order = Order.new(currency: 'ltcbtc', price: nil, ord_type: 'limit')
      order.should_not be_valid
      order.errors[:price].should == ["is not a number"]
    end

    it "should make sure price is greater than zero" do
      order = Order.new(currency: 'ltcbtc', price: '0.0'.to_d, ord_type: 'limit')
      order.should_not be_valid
  ...
  context "market order" do
    it "should make sure price is not present" do
      order = Order.new(currency: 'ltcbtc', price: '0.0'.to_d, ord_type: 'market')
      order.should_not be_valid
...
describe Order, "#fix_number_precision" do
  let(:order_bid) { create(:order_bid, currency: 'ltcbtc', price: '12.326'.to_d, volume: '123.123456789') }
  let(:order_ask) { create(:order_ask, currency: 'ltcbtc', price: '12.326'.to_d, volume: '123.123456789') }
